...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.a_feature_size = opt.a_feature_size#1536
        self.m_feature_size = opt.m_feature_size#1024
        self.hidden_size = opt.hidden_size
        self.concat_size = self.a_feature_size + self.m_feature_size
        self.use_multi_gpu = opt.use_multi_gpu
        self.node_num=2
        self.cluter_center = nn.Parameter(torch.FloatTensor(16,opt.hidden_size))
        self.language_model = Bert()
        
        self.trans1= nn.Linear(self.concat_size, self.concat_size//2)
        self.trans2= nn.Linear(self.concat_size, self.concat_size//2)
        self.trans3= nn.Linear(self.concat_size*2, self.concat_size)
        # frame feature embedding
        self.frame_feature_embed = nn.Linear(self.concat_size, self.hidden_size)
        self.cal_loss=ContrastiveLoss()
        self.cal_loss_tri=Tri_Loss()
        self.cal_loss_cos=ContrastiveLoss_cos()
        self.creat_tri=Creat_Tri_Loss()
        self.cap_dim=26
        self.word_size = opt.word_size
        self.vocab_size=opt.vocab_size
        self.rnn_size=1024
        self.cap_emb=nn.Linear(self.word_size,self.rnn_size)
        self.cap_trans=nn.Linear(self.rnn_size,self.concat_size)
        self.word_embed = nn.Embedding(self.vocab_size, self.word_size)
        self.expa=nn.Linear(self.concat_size,26*self.concat_size)
    ...
    def forward(self, cnn_feats,sentence=None,mode="train"):
        ...
        if mode=="train": 
            ...
            text_features, hidden_states = self.language_model(tokens, segments, input_masks)#torch.Size([128, 768]),torch.Size([128, 99, 768])
            text_features=self.bert_emb(text_features)##torch.Size([128, 1024])
