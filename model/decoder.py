...
class Decoder(nn.Module):
    def __init__(self, opt, filed):
        ...
        elif opt.attention =="self":
            self.self_att=SelfAttention(self.hidden_size, self.hidden_size)
        # elif opt.attention=="multihead":
        ...
        self.output_dropout=nn.Dropout(p=opt.dropout)
    ...
    def init_weights(self):
        self.word_embed.weight.data.copy_(torch.tensor(self.field.vocab.vectors))
        ...
        if not infer or self.beam_size == 1:
            for i in range(self.max_words):
                ...
                if use_teacher_forcing:
                    ...
                    if self.mask=="True":
                        for mm in range(self.batch_size):
                            id=int(pre_id[mm])
                            if int(id) !=0 and int(id) !=2:
                                word_logits[mm][id]=0
    ...
    def decode_tokens(self, tokens):
        ...
        for token in tokens:
            ...
            if self.dataset=="msvd":
                word = self.field.vocab.itos[str(int(token))]
            elif self.dataset=="msr-vtt":
                word = self.field.vocab.itos[int(token)]
            words.append(word)
