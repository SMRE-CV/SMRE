...
def main(opt):
    ...
    if opt.dataset=="msvd":
        filed.vocab=pickle.load(open("data/MSVD/vocab_refine.pkl", 'rb'))
    elif opt.dataset=="msr-vtt":
        filed.vocab = pickle.load(open("data/MSR-VTT/vocab_2.pkl",'rb'))
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (frames, captions, cap_lens, video_ids,sent) in enumerate(train_loader, start=1):
            ...
            if i % 100 == 0 or bsz < opt.train_batch_size:
                ...
                # tokens_pos = tokens_pos.max(2)[1]
                # tokens_pos = tokens_pos.data[0].squeeze()
                if opt.use_multi_gpu:
                ...
                if opt.choose=="sota":
                    ...
                    # logger.write("cap_loss:{},loss_contrastive:{}".format(cap_loss,loss_contrastive))
                else:
